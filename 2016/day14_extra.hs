...
stretched :: Hash -> Hash
stretched (Hash i str) = Hash i (rehash 2016 str)
    where
        rehash :: Integer -> BS.ByteString -> BS.ByteString
        rehash i str
            | i <= 0 = str
            | otherwise = rehash (i-1) ((B16.encode . hash) str)
